<?php
//Konfigurationen
function ps($sql, $param_array=array()) {
...
function make_booster($editionshort) {
    $booster = array();
    $booster[] = get_random_card($editionshort, "rare");
    for ($i = 0; $i < 3; $i++) {
        //doppelte verhindern
        $newcard = get_random_card($editionshort, "uncommon");
        if (!in_array($newcard, $booster)){
            $booster[] = $newcard;
        }
        else {
            $i--;
    ...
    for ($i = 0; $i < 11; $i++) {
        //doppelte verhindern
        $newcard = get_random_card($editionshort, "common");
        if (!in_array($newcard, $booster)){
            $booster[] = $newcard;
        }
        else {
...
function get_random_card($editionshort, $rarity){
    $table = "cardlinktable";
    $sql = "SELECT picturelink FROM $table WHERE edition= ? AND rarity = ? ORDER BY RAND() LIMIT 1";
    $result = ps($sql,array($editionshort, $rarity));
...
if(isset($_GET['edition'])) {
    ...
    //echo get_random_card("wwk/de", "rare");
    
    $booster = make_booster($_GET['edition']);
